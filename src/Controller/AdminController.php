...
class AdminController extends AbstractController
{
    ...
    public function dashboard(
        ...
        $comptesBloques = $compteRepo->count(['statut' => CompteBancaire::STATUS_BLOQUE]);
        $recentTransactions = $transactionRepo->findBy([], ['createdAt' => 'DESC'], 8);
    ...
    {
        if (!$this->isCsrfTokenValid('delete_user_' . $client->getId(), (string) $request->request->get('_token'))) {
    ...
    {
        if (!$this->isCsrfTokenValid('toggle_compte_' . $compte->getId(), (string) $request->request->get('_token'))) {
            $this->addFlash('error', 'Token CSRF invalide.');
